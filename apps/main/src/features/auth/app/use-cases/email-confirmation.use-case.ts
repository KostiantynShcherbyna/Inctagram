...
export class EmailConfirmationUseCase
	...
	async execute(command: EmailConfirmationCommand) {
		...
		if (confirmationCode === null)
			return ErrorEnum.CONFIRMATION_CODE_NOT_FOUND

		const user = await this.usersRepository
			.findUserById(confirmationCode.userId)
		...
		if (confirmationCodeDto === null) return ErrorEnum.INVALID_TOKEN

		const updateResult = await this.usersRepository
			.updateConfirmation(user.id, true)

		if (!updateResult) return ErrorEnum.USER_NOT_FOUND
