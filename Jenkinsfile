...
pipeline {
    ...
    environment {
        ENV_TYPE = "production"
        PORT = 3153
        NAMESPACE = "visualvoyage-ru"
        REGISTRY_HOSTNAME = "ilmirrahkmatullin"
        PROJECT = "incstagram-back"
        SERVICE="main"
        REGISTRY = "registry.hub.docker.com"
        DEPLOYMENT_NAME = "incstagram-back-deployment"
        ...
        stage('Push docker image') {
             steps {
                 echo "Push image started..."
                     script {
                        docker.withRegistry("https://${env.REGISTRY}", 'visualvoyage-ru') {
